...
async function main(){
  const archimedean_spiral = new ArchimedeanSpiral();
  await archimedean_spiral.init();
  ...
  function animationLoop(time_stamp) {
    ...
    archimedean_spiral.render(time_stamp);
  ...
  function resizeCanvas() {
    ...
    gl.viewport(0, 0, c.width, c.height);
    archimedean_spiral.setProjectionMatrix(glm.ortho(-c.width / 2, c.width / 2, -c.height / 2, c.height / 2));
...
function ArchimedeanSpiral(){
  ...
  self.render = function(time_stamp){
    ...
    var delta_x = (c.width * 0.47) / number_of_points;
    var angle = 2 * Math.PI - (time_stamp / 3000) % (2 * Math.PI);
    var angle_offset = 0;
    // var delta_angle = (2 * Math.PI) / 4;
    var delta_angle = sliderAngle.valueAsNumber;
    // console.log("sliderAngle.value - " + sliderAngle.value);

    for (x = 0; x <= (c.width * 0.47); x += delta_x) {
      model_matrix = glm.mat4(1.0);
      // model_matrix = glm.translate(model_matrix, glm.vec3(x * Math.cos(angle + x / 2), x * Math.sin(angle + x / 2), 0));
      model_matrix = glm.translate(model_matrix, glm.vec3(x * Math.cos(angle + angle_offset), x * Math.sin(angle + angle_offset), 0));
